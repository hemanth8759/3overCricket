...
if(t1score>t2score){
    alert("TEAM 1 Wins")
    document.getElementById("tc").innerHTML="player "+(op.indexOf(Math.max(...op))+1)+ " is the Man of the Match With Score " + Math.max(...op)
}else{
    alert("TEAM 2 Wins")
    document.getElementById("tc").innerHTML="player "+(po.indexOf(Math.max(...po))+1)+ " is the Man of the Match With Score "+ Math.max(...po)
